...
def train_split_learning(num_epochs=3, batch_size=32, data_dir="../data/MNIST", dataset_name="MNIST"):
	client = ClientNet()
	server = ServerNet()
	optimizer_client = optim.SGD(client.parameters(), lr=0.01)
	optimizer_server = optim.SGD(server.parameters(), lr=0.01)
	loader = load_dataset(dataset_name, data_dir, batch_size=batch_size, train=True)
...
def evaluate(client, server, data_dir="../data/MNIST", batch_size=32, dataset_name="MNIST"):
