function acc = fedyogi(x_train, y_train, x_test, y_test, varargin)
% FedYogi: Server-side Yogi optimizer.
% Optional name-value similar to FedAdam.
    ...
    for t = 1:rounds
        ...
        for j = 1:numel(global_weights)
            ...
            v_new = v_t{j} - (1 - beta2) * sign(v_t{j} - delta{j}.^2) .* (delta{j}.^2);
            m_hat = m_new ./ (1 - beta1^t);
            v_hat = v_new ./ (1 - beta2^t);
            new_global_weights{j} = global_weights{j} - server_lr * m_hat ./ (sqrt(abs(v_hat)) + tau);
            m_t{j} = m_new; v_t{j} = v_new;
        end
        global_weights = new_global_weights;
        global_model.set_weights(global_weights);

        [metrics, prev_acc, prev_global_weights, comm_cost] = compute_round_metrics('FedYogi', t, start_time, global_model, x_test, y_test, prev_acc, prev_global_weights, global_weights, clients, comm_cost, 'local_weights_list', client_weights, 'reference_weights', avg_weights);
        if ~isempty(report), report('FedYogi', t, metrics); end
    end
